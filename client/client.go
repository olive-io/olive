...
import (
	...
	"github.com/olive-io/olive/client/internal/resolver"
...
var (
	ErrNoAvailableEndpoints = errors.New("oliveclient: no available endpoints")
)
...
type Client struct {
	...
	resolver *resolver.OliveManualResolver
	mu       *sync.RWMutex
...
// New creates a new olive client from a given configuration.
...
// NewFromURL creates a new olive client from a URL.
func NewFromURL(url string) (*Client, error) {
	return New(Config{Endpoints: []string{url}})
}

// NewFromURLs creates a new olive client from URLs.
...
//
// Deprecated: Please use WithZapLogger or Logger field in client.Config
//
...
// NOTE: This method is for internal use of olive-client library and should not be used as general-purpose logger.
...
// Sync synchronizes client's endpoints with the known endpoints from the olive membership.
...
func newClient(cfg *Config) (*Client, error) {
	...
	if cfg.Logger != nil {
		...
		client.lg, err = logutil.CreateDefaultZapLogger(oliveClientDebugLevel())
		if client.lg != nil {
			client.lg = client.lg.Named("olive-client")
		}
	}
	if err != nil {
		return nil, err
