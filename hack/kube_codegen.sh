...
#
# USAGE: kube::codegen::gen_protobuf [FLAGS] <input-dir>
#
# FLAGS:
#
#   --boilerplate <string = path_to_kube_codegen_boilerplate>
#     An optional override for the header file to insert into generated files.
#
function kube::codegen::gen_protobuf() {
    local in_dir=""
    local out_dir=""
    local report="/dev/null"
    local update_report=""
    ...
    if [ -z "${out_dir}" ]; then
        echo "output-dir argument is required" >&2
        return 1
    fi
    ...
    if [ "${#input_pkgs[@]}" != 0 ]; then
        echo "Generating protobuf code for ${#input_pkgs[@]} targets"
