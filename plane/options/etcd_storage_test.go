...
func TestEtcdStorageOptionsValidate(t *testing.T) {
	testCases := []struct {
		name        string
		testOptions *EtcdStorageOptions
		expectErr   string
	}{
		{
		...
		{
			name: "test when EtcdStorageOptions is valid",
			testOptions: &EtcdStorageOptions{
				StorageConfig: storagebackend.Config{
					Type:   "etcd3",
					Prefix: "/registry",
...
func TestKMSHealthzEndpoint(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			etcdOptions := &EtcdStorageOptions{
...
func TestReadinessCheck(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			serverConfig := server.NewConfig(codecs)
			etcdOptions := &EtcdStorageOptions{SkipHealthEndpoints: tc.skipHealth}
			if err := etcdOptions.ApplyTo(serverConfig); err != nil {
				t.Fatalf("Failed to add healthz error: %v", err)
			}
