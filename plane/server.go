...
import (
	...
	monscheduler "github.com/olive-io/olive/plane/scheduler"
)
...
func (s *PlaneServer) InstallAPIs(apiResourceConfigSource serverstorage.APIResourceConfigSource, restOptionsGetter generic.RESTOptionsGetter, restStorageProviders ...RESTStorageProvider) error {
	nonLegacy := []*genericapiserver.APIGroupInfo{}

	// used later in the loop to filter the served resource by those that have expired.
	resourceExpirationEvaluator, err := genericapiserver.NewResourceExpirationEvaluator(s.genericAPIServer.EffectiveVersion.BinaryVersion())
	if err != nil {
		return err
	}
...
func (s *PlaneServer) stop() error {
	s.lg.Debug("Shutdown olive-plane")
	s.IDaemon.Shutdown()
	return nil
}

func (s *PlaneServer) destroy() {
	s.lg.Debug("Stopping olive-plane")
	s.cancel()
