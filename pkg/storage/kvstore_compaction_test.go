...
func TestScheduleCompaction(t *testing.T) {
	...
	for i, tt := range tests {
		...
		s := NewStore(zaptest.NewLogger(t), b, StoreConfig{})
		fi := newFakeIndex()
		...
		for _, rev := range tt.wrevs {
			revToBytes(rev, ibytes)
			keys, _, _ := tx.UnsafeRange(buckets.Key, ibytes, nil, 0)
			if len(keys) != 1 {
				t.Errorf("#%d: range on %v = %d, want 1", i, rev, len(keys))
			}
		}
		_, vals, _ := tx.UnsafeRange(buckets.Meta, finishedCompactKeyName, nil, 0)
...
func TestCompactAllAndRestore(t *testing.T) {
	...
	s1 := NewStore(zap.NewExample(), b, StoreConfig{})
	if s1.Rev() != rev {
