...
func (s *store) Restore(b backend.IBackend) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *store) restore() error {
	...
	_, finishedCompactBytes, _ := tx.UnsafeRange(buckets.Meta, finishedCompactKeyName, nil, 0)
	...
	_, scheduledCompactBytes, _ := tx.UnsafeRange(buckets.Meta, scheduledCompactKeyName, nil, 0)
	...
	for {
		keys, vals, _ := tx.UnsafeRange(buckets.Key, min, max, int64(restoreChunkKeys))
		if len(keys) == 0 {
			break
		}
...
func (s *store) HashStorage() IHashStorage {
	return s.hashes
}
