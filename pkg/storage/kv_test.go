...
import (
	...
	"github.com/oliveio/olive/client/pkg/testutil"
	"github.com/oliveio/olive/pkg/storage/backend"
	betesting "github.com/oliveio/olive/pkg/storage/backend/testing"
...
type (
	rangeFunc       func(kv IKV, key, end []byte, ro RangeOptions) (*RangeResult, error)
	putFunc         func(kv IKV, key, value []byte) int64
	deleteRangeFunc func(kv IKV, key, end []byte) (n, rev int64)
)

var (
	normalRangeFunc = func(kv IKV, key, end []byte, ro RangeOptions) (*RangeResult, error) {
		return kv.Range(context.TODO(), key, end, ro)
	}
	txnRangeFunc = func(kv IKV, key, end []byte, ro RangeOptions) (*RangeResult, error) {
		txn := kv.Read(ConcurrentReadTxMode, traceutil.TODO())
		defer txn.End()
		return txn.Range(context.TODO(), key, end, ro)
	}

	normalPutFunc = func(kv IKV, key, value []byte) int64 {
		return kv.Put(key, value)
	}
	txnPutFunc = func(kv IKV, key, value []byte) int64 {
		txn := kv.Write(traceutil.TODO())
		defer txn.End()
		return txn.Put(key, value)
	}

	normalDeleteRangeFunc = func(kv IKV, key, end []byte) (n, rev int64) {
		return kv.DeleteRange(key, end)
	}
	txnDeleteRangeFunc = func(kv IKV, key, end []byte) (n, rev int64) {
		txn := kv.Write(traceutil.TODO())
		defer txn.End()
...
func testKVPutMultipleTimes(t *testing.T, f putFunc) {
	...
	for i := 0; i < 10; i++ {
		...
		wkvs := []api.KeyValue{
			{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 2, ModRevision: base + 1, Version: base},
		}
...
func TestKVCompactReserveLastValue(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			1,
			[]api.KeyValue{
				{Key: []byte("foo"), Value: []byte("bar0"), CreateRevision: 2, ModRevision: 2, Version: 1},
			},
		},
		{
			2,
			[]api.KeyValue{
				{Key: []byte("foo"), Value: []byte("bar1"), CreateRevision: 2, ModRevision: 3, Version: 2},
			},
		},
		{
			3,
		...
		},
		{
			4,
			[]api.KeyValue{
				{Key: []byte("foo"), Value: []byte("bar2"), CreateRevision: 5, ModRevision: 5, Version: 1},
			},
		},
	}
	for i, tt := range tests {
...
func TestKVHash(t *testing.T) {
	...
	for i := 0; i < len(hashes); i++ {
		var err error
		b, tmpPath := betesting.NewDefaultTmpBackend(t)
		kv := NewStore(zap.NewExample(), b, StoreConfig{})
...
func TestKVRestore(t *testing.T) {
	...
	for i, tt := range tests {
		...
		ns := NewStore(zap.NewExample(), b, StoreConfig{})
...
func cleanup(s IKV, b backend.IBackend, path string) {
	s.Close()
	b.Close()
	os.Remove(path)
}

func put3TestKVs(s IKV) []api.KeyValue {
	s.Put([]byte("foo"), []byte("bar"))
	s.Put([]byte("foo1"), []byte("bar1"))
	s.Put([]byte("foo2"), []byte("bar2"))
	return []api.KeyValue{
