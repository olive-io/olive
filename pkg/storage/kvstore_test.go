...
func TestRestoreContinueUnfinishedCompaction(t *testing.T) {
	...
	for _, test := range tests {
		...
		for i := 0; i < 5; i++ {
			tx = s.b.BatchTx()
			tx.Lock()
			ks, _, _ := tx.UnsafeRange(buckets.Key, revbytes, nil, 0)
			tx.Unlock()
			if len(ks) != 0 {
				time.Sleep(100 * time.Millisecond)
				continue
...
func newFakeStore() *store {
	...
	s.IReadView, s.IWriteView = &readView{s}, &writeView{s}
...
func (b *fakeBatchTx) UnsafeSeqPut(bucket backend.IBucket, key []byte, value []byte) {
	b.Recorder.Record(testutil.Action{Name: "seqput", Params: []interface{}{bucket, key, value}})
}
func (b *fakeBatchTx) UnsafeRange(bucket backend.IBucket, key, endKey []byte, limit int64) (keys [][]byte, vals [][]byte, err error) {
	b.Recorder.Record(testutil.Action{Name: "range", Params: []interface{}{bucket, key, endKey, limit}})
	r := <-b.rangeRespc
	return r.keys, r.vals, nil
}
func (b *fakeBatchTx) UnsafeDelete(bucket backend.IBucket, key []byte) {
	b.Recorder.Record(testutil.Action{Name: "delete", Params: []interface{}{bucket, key}})
}
func (b *fakeBatchTx) UnsafeForEach(bucket backend.IBucket, visitor func(k, v []byte) error) error {
	return nil
}
...
func (b *fakeBackend) BatchTx() backend.IBatchTx                                  { return b.tx }
func (b *fakeBackend) ReadTx() backend.IReadTx                                    { return b.tx }
func (b *fakeBackend) ConcurrentReadTx() backend.IReadTx                          { return b.tx }
...
func (b *fakeBackend) Snapshot() backend.ISnapshot                                { return nil }
func (b *fakeBackend) Recover(reader io.Reader) error                             { return nil }
