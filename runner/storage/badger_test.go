...
func newStorage(t *testing.T) (storage.Storage, func()) {
	cfg := storage.NewConfig("tmp")
	b, err := storage.NewStorage(cfg)
	if err != nil {
...
func TestStorageMarshal(t *testing.T) {
	b, cancel := newStorage(t)
	defer cancel()
	...
	var td TestData
	if err = storage.NewUnmarshaler[TestData](resp).To(&td); err != nil {
		t.Fatalf("error unmarshaling value: %v", err)
	}
	assert.Equal(t, td.Field1, td1.Field1)

	resp, err = b.Get(ctx, "foo", storage.GetPrefix())
	if err != nil {
		t.Fatalf("error getting value: %v", err)
	}

	ts := make([]TestData, 0)
	if err = storage.NewUnmarshaler[TestData](resp).SliceTo(&ts); err != nil {
		t.Fatalf("error unmarshaling value: %v", err)
	}
