...
ROOT_MODULE="github.com/olive-io/olive"
...
function set_root_dir {
  OLIVE_ROOT_DIR=$(go list -f '{{.Dir}}' "${ROOT_MODULE}")
}
...
function module_subdir {
  relativePath "${OLIVE_ROOT_DIR}" "${PWD}"
}
...
function run_for_module {
  ...
  shift 1
  (
    cd "${OLIVE_ROOT_DIR}/${module}" && "$@"
  )
}

function module_dirs() {
  echo "api pkg pkg/tonic client meta runner gateway tests ."
}
