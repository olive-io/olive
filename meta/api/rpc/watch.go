...
func GetProgressReportInterval() time.Duration {
	...
	// add rand(1/10*progressReportInterval) as jitter so that olive server will not
...
// serverWatchStream is an olive server side stream. It receives requests
...
func IsCreateEvent(e pb.Event) bool {
	return e.Type == pb.Event_PUT && e.Kv.CreateRevision == e.Kv.ModRevision
}
...
func sendFragments(
	...
	if wr.XSize() < maxRequestBytes || len(wr.Events) < 2 {
		return sendFunc(wr)
	}

	ow := *wr
	ow.Events = make([]*pb.Event, 0)
	ow.Fragment = true
	...
	for {
		...
		for _, ev := range wr.Events[idx:] {
			...
			if len(cur.Events) > 1 && cur.XSize() >= maxRequestBytes {
...
func filterNoDelete(e pb.Event) bool {
	return e.Type == pb.Event_DELETE
}

func filterNoPut(e pb.Event) bool {
	return e.Type == pb.Event_PUT
}
