...
func TestSyncWatchers(t *testing.T) {
	...
	if evs[0].Type != pb.Event_PUT {
		t.Errorf("got = %v, want = %v", evs[0].Type, pb.Event_PUT)
	}
...
func readEventsForSecond(ws <-chan WatchResponse) (events []pb.Event) {
	for {
		select {
		case resp := <-ws:
...
func TestNewMapwatcherToEventMap(t *testing.T) {
	...
	evs := []pb.Event{
		{
			Type: pb.Event_PUT,
			Kv:   &pb.KeyValue{Key: k0, Value: v0},
		},
		{
			Type: pb.Event_PUT,
			Kv:   &pb.KeyValue{Key: k1, Value: v1},
		},
		{
			Type: pb.Event_PUT,
			Kv:   &pb.KeyValue{Key: k2, Value: v2},
		},
	}
