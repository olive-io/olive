...
func TestWatcherWatchID(t *testing.T) {
	...
	s := IWatchableKV(newWatchableStore(zap.NewExample(), b, &lease.FakeLessor{}, StoreConfig{}))
...
func TestWatchDeleteRange(t *testing.T) {
	...
	we := []pb.Event{
		{Type: pb.Event_DELETE, Kv: &pb.KeyValue{Key: []byte("foo_0"), ModRevision: 5}},
		{Type: pb.Event_DELETE, Kv: &pb.KeyValue{Key: []byte("foo_1"), ModRevision: 5}},
		{Type: pb.Event_DELETE, Kv: &pb.KeyValue{Key: []byte("foo_2"), ModRevision: 5}},
	}
...
func TestWatcherRequestProgressAll(t *testing.T) {
	...
	w.RequestProgressAll()
	wrs := WatchResponse{WatchID: client.InvalidWatchID, Revision: 2}
	select {
	case resp := <-w.Chan():
		if !reflect.DeepEqual(resp, wrs) {
			t.Fatalf("got %+v, expect %+v", resp, wrs)
...
func TestWatcherWatchWithFilter(t *testing.T) {
	...
	filterPut := func(e pb.Event) bool {
