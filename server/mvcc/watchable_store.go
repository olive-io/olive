...
func New(lg *zap.Logger, b backend.IBackend, le lease.ILessor, cfg StoreConfig) IWatchableKV {
	return newWatchableStore(lg, b, le, cfg)
}

func newWatchableStore(lg *zap.Logger, b backend.IBackend, le lease.ILessor, cfg StoreConfig) *watchableStore {
	if lg == nil {
		lg = zap.NewNop()
	}
	s := &watchableStore{
	...
	s.store.IReadView = &readView{s}
	s.store.IWriteView = &writeView{s}
	if s.le != nil {
...
func (s *watchableStore) NewWatchStream() IWatchStream {
...
func (s *watchableStore) Restore(b backend.IBackend) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *watchableStore) syncWatchers() int {
	...
	tx := s.store.b.ReadTx()
	tx.RLock()
	revs, vs, _ := tx.UnsafeRange(buckets.Key, minBytes, maxBytes, 0)
...
func kvsToEvents(lg *zap.Logger, wg *watcherGroup, revs, vals [][]byte) (evs []pb.Event) {
	for i, v := range vals {
		var kv pb.KeyValue
		if err := kv.Unmarshal(v); err != nil {
			lg.Panic("failed to unmarshal pb.KeyValue", zap.Error(err))
		}
		...
		evs = append(evs, pb.Event{Kv: &kv, Type: ty})
	}
	return evs
}
...
func (s *watchableStore) notify(rev int64, evs []pb.Event) {
...
func (s *watchableStore) progressAll(watchers map[WatchID]*watcher) bool {
	return s.progressIfSync(watchers, client.InvalidWatchID)
}
...
func (w *watcher) send(wr WatchResponse) bool {
	...
	if len(w.fcs) != 0 {
		ne := make([]pb.Event, 0, len(wr.Events))
