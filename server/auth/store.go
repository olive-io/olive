...
import (
	...
	"github.com/olive-io/olive/api/authpb"
...
func (as *authStore) Recover(be backend.IBackend) {
	enabled := false
	as.be = be
	tx := be.ReadTx()
	tx.Lock()
...
func (as *authStore) selectPassword(password string, hashedPassword string) ([]byte, error) {
	if password != "" && hashedPassword == "" {
		// This path is for processing log entries created by olive whose version is older than 3.5
...
func (as *authStore) UserChangePassword(r *pb.AuthUserChangePasswordRequest) (*pb.AuthUserChangePasswordResponse, error) {
	...
	// Backward compatible with old versions of olive, user options is nil
	if user.Options == nil || !user.Options.NoPassword {
		password, err = as.selectPassword(r.Password, r.HashedPassword)
		if err != nil {
			return nil, ErrNoPasswordUser
...
func getUser(lg *zap.Logger, tx backend.IReadTx, username string) *authpb.User {
	_, vs, _ := tx.UnsafeRange(buckets.AuthUsers, []byte(username), nil, 0)
	if len(vs) == 0 {
		return nil
	}
...
func getAllUsers(lg *zap.Logger, tx backend.IReadTx) []*authpb.User {
	var vs [][]byte
...
func putUser(lg *zap.Logger, tx backend.IBatchTx, user *authpb.User) {
	b, err := user.Marshal()
	if err != nil {
		lg.Panic("failed to unmarshal 'authpb.User'", zap.Error(err))
	}
...
func delUser(tx backend.IBatchTx, username string) {
	tx.UnsafeDelete(buckets.AuthUsers, []byte(username))
}

func getRole(lg *zap.Logger, tx backend.IReadTx, rolename string) *authpb.Role {
	_, vs, _ := tx.UnsafeRange(buckets.AuthRoles, []byte(rolename), nil, 0)
	if len(vs) == 0 {
		return nil
	}
...
func getAllRoles(lg *zap.Logger, tx backend.IReadTx) []*authpb.Role {
	_, vs, _ := tx.UnsafeRange(buckets.AuthRoles, []byte{0}, []byte{0xff}, -1)
	if len(vs) == 0 {
		return nil
	}
...
func putRole(lg *zap.Logger, tx backend.IBatchTx, role *authpb.Role) {
	b, err := role.Marshal()
	if err != nil {
		lg.Panic(
...
func delRole(tx backend.IBatchTx, rolename string) {
...
func NewAuthStore(lg *zap.Logger, be backend.IBackend, tp TokenProvider, bcryptCost int) *authStore {
	if lg == nil {
		lg = zap.NewNop()
	}
...
func (as *authStore) commitRevision(tx backend.IBatchTx) {
...
func getRevision(tx backend.IReadTx) uint64 {
	_, vs, _ := tx.UnsafeRange(buckets.Auth, revisionKey, nil, 0)
	if len(vs) != 1 {
...
func (as *authStore) AuthInfoFromTLS(ctx context.Context) (ai *AuthInfo) {
	...
	for _, chains := range tlsInfo.State.VerifiedChains {
		...
		// gRPC-gateway proxy request to olive server includes Grpcgateway-Accept
		// header. The proxy uses olive client server certificate. If the certificate
...
func (as *authStore) WithRoot(ctx context.Context) context.Context {
	...
	// use "mdIncomingKey{}" since it's called from local olive server
