...
func TestLessorGrant(t *testing.T) {
	...
	be.BatchTx().Lock()
	_, vs, _ := be.BatchTx().UnsafeRange(buckets.Lease, int64ToBytes(int64(l.ID)), nil, 0)
	if len(vs) != 1 {
...
func TestLessorRenewWithCheckpointer(t *testing.T) {
	...
	fakerCheckerpointer := func(ctx context.Context, cp *pb.LeaseCheckpointRequest) {
		for _, cp := range cp.Checkpoints {
			le.Checkpoint(LeaseID(cp.ID), cp.Remaining_TTL)
		}
	}
	defer le.Stop()
	// Set checkpointer
...
func newFakeDeleter(be backend.IBackend) *fakeDeleter {
...
func NewTestBackend(t *testing.T) (string, backend.IBackend) {
	tmpPath, err := os.MkdirTemp("", "lease")
	if err != nil {
		t.Fatalf("failed to create tmpdir (%v)", err)
	}
	bcfg := backend.DefaultBackendConfig()
	bcfg.Dir = filepath.Join(tmpPath, "be")
