...
// ILessor owns leases. It can grant, revoke, renew and modify leases for lessee.
type ILessor interface {
	...
	Recover(b backend.IBackend, rd RangeDeleter)
...
func (le *lessor) Recover(b backend.IBackend, rd RangeDeleter) {
	le.mu.Lock()
	defer le.mu.Unlock()
...
func (fl *FakeLessor) Recover(b backend.IBackend, rd RangeDeleter) {}
